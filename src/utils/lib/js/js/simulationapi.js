...
export const dofilter = (allfilter = {}) => {
  let data = []
  const filter = allfilter.filters
  const page = allfilter.page
  for (let i = 0; i < allusers.length; i++) {
    let sameKeyNum = 0
  ...
  CUR_FILTERDATA = data
  const rdata = CUR_FILTERDATA.slice(perpagecount * (page - 1), perpagecount * (page - 1) + perpagecount)
  const _page = Math.ceil(data.length / perpagecount) ? Math.ceil(data.length / perpagecount) : 1
  const res = {
    data: rdata,
    pagenum: _page
  }
...
export const createTableDataByRandom = (len) => {
  ...
  for (let i = 0; i < len; i++) {
    let curuser = {}
    const cursing = csign[Math.floor(Math.random() * csign.length)]

    const curx = xarr[Math.floor(Math.random() * xarr.length)]
    const curn = nchar[Math.floor(Math.random() * nchar.length)]
    let cury = 1996
    const curm = Math.floor(Math.random() * 11) + 1
    const curd = Math.floor(Math.random() * 29) + 1
    const gender = gchar[Math.floor(Math.random() * gchar.length)]
    ...
    const province = bcity[Math.floor(Math.random() * bcity.length)]
    const city = province.children[Math.floor(Math.random() * province.children.length)]
    const birthPlace = JSON.parse(JSON.stringify(province))
  ...
  const _page = Math.ceil(users.length / perpagecount) ? Math.ceil(users.length / perpagecount) : 1
  const rdata = users.slice(0, perpagecount)
  const res = {
    data: rdata,
    pagenum: _page
  }
